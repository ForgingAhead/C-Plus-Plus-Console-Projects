...
void processMenuEvents(int option) {
	...
	switch (option) {                    
        ...
        case REVERSE: if(clk) clk = 0; else clk = 1;break;
        case CLOCKWISE :  clk = 1;break;
        case ANTI_CLOCKWISE :  clk = 0; break;
...
display(void)
{
    ...
    {       
        if(speed)
        {
            if(speed>0)
            {
                angle += 0.090f * (speed+1)*((float) dt); //increase speed 
                if(angle>360.0)angle -=360.0f;
            }
            else     
            {
                angle += (0.090f/(-speed+1)) * ((float) dt); /* decrease speed */
                if (angle >  360.0f) angle -= 360.0f;
            }
        }
        else
        {        
            angle += 0.090f* ((float) dt); 
            if (angle >  360.0f) angle -= 360.0f;   //rotate in the original speed
        }            
    }
    ...
    }else glRotatef(angle, 0.0f, 0.0f, 1.0f);
