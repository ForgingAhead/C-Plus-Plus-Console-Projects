...
/*change the square size to enable distinguishes between the mouse button pressed*/

void mouse(int btn, int state, int x, int y)
{
    if(btn==GLUT_RIGHT_BUTTON && state== GLUT_UP)   exit(0);// quit while the right mouse button is released;
    if(btn == GLUT_LEFT_BUTTON && state == GLUT_DOWN) size = 2.0; 
    if(btn == GLUT_RIGHT_BUTTON && state == GLUT_DOWN) size = 11.0; 
    if(btn == GLUT_MIDDLE_BUTTON) size = 7.0;
}


/*enable to quit from the program using 'q' & 'Q'*/
